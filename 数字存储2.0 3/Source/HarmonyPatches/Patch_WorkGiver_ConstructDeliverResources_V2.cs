...
namespace DigitalStorage.HarmonyPatches
{
    ...
    public static class Patch_WorkGiver_ConstructDeliverResources_V2
    {
            ...
            bool forced)
        {
            ...
            // ⚠️ 注意：移除全局同步，允许多个pawn同时处理同一个建造目标
            // 因为建造任务通常需要多种材料，多个pawn可以同时工作
            ...
            foreach (ThingDefCountClass need in c.TotalMaterialCost())
            {
                ...
                else if (reservedInsufficient)
                {
                    if (hasVirtualStorage)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Log.Message($"[数字存储] ResourceDeliverJobFor: 预留不够且无访问权限，让原版处理, 预留={reservedCount}, 需要={countNeeded}");
                            }
                            // 让原版逻辑处理
                            return;
                        }
                    ...
                    else
                    {
                        ...
                        if (DigitalStorageSettings.enableDebugLog)
                        {
                            Log.Message($"[数字存储] ResourceDeliverJobFor: 预留不够且虚拟存储无，让原版处理, 预留={reservedCount}, 需要={countNeeded}");
                        }
                        // 让原版逻辑处理
                        return;
                    }
                ...
                else
                {
                        ...
                        if (hasChip || hasInterface)
                        {
                            ...
                            if (replenishedThing != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (DigitalStorageSettings.enableDebugLog)
                                    {
                                        Log.Message($"[数字存储] ResourceDeliverJobFor: 补货后仍不够且无访问权限，让原版处理, 预留={newReservedThing?.stackCount ?? 0}, 需要={countNeeded}");
                                    }
                                    // 让原版逻辑处理
                                    return;
                                    }
                        ...
                        else
                        {
                            ...
                            if (replenishedThing != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (DigitalStorageSettings.enableDebugLog)
                                    {
                                        Log.Message($"[数字存储] ResourceDeliverJobFor: 补货后仍不够，让原版处理, 预留={newReservedThing?.stackCount ?? 0}, 需要={countNeeded}");
                                    }
                                    // 让原版逻辑处理
                                    return;
                                }
                            ...
                            else
                            {
                                ...
                                if (DigitalStorageSettings.enableDebugLog)
                                {
                                    Log.Message($"[数字存储] ResourceDeliverJobFor: 补货失败且无访问权限，让原版处理, 需要={countNeeded}");
                                }
                                    // 让原版逻辑处理
                                    return;
                            }
                    ...
                    else
                    {
                        ...
                        if (DigitalStorageSettings.enableDebugLog)
                        {
                            Log.Message($"[数字存储] ResourceDeliverJobFor: 无预留且虚拟存储无，让原版处理, 需要={countNeeded}");
                        }
                        // 让原版逻辑处理
                        return;
                    }
