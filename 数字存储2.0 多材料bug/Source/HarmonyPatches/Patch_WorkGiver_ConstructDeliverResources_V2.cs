            ...
            bool forced)
        {
            ...
            foreach (ThingDefCountClass need in c.TotalMaterialCost())
            {
                ...
                if (materialCore != null)
                {
                    ...
                    // 补货到需要的数量
                    Thing replenishedThing = ReplenishToNeededCount(materialCore, need.thingDef, null, countNeeded, replenishPos, pawn.Map, reservedThing);
                    ...
                    if (replenishedThing != null && replenishedThing.Spawned)
                    {
                        ...
                        // 使用传送后的物品创建Job
                        __result = CreateTeleportJobForRealThing(pawn, c, replenishedThing, countNeeded);
                        return;
                    }
                    else
                    {
