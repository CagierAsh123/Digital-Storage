...
namespace DigitalStorage.Components
{
    ...
    public class Building_StorageCore : Building_Storage, IRenameable
    {
                ...
                if (slotGroup != null)
                {
                    ...
                    foreach (Thing thing in slotGroup.HeldThings)
                    {
                        ...
                        if (itemCounts[key] > reservedCount)
                        {
                            ...
                            if (DigitalStorageSettings.enableConversionLog)
                            {
                                Log.Message($"[数字存储] Tick检查: 发现超出预留的物品, Key={key}, 数量={itemCounts[key]}, 预留={reservedCount}");
                            }
                            break;
                        }
                    }
        ...
        private void MaintainReservedItems()
        {
            ...
            foreach (StoredItemData itemData in this.virtualStorage)
            {
                ...
                // 获取当前实际预留数量（如果不存在就是0）
                int currentCount = currentPhysicalCounts.ContainsKey(key) ? currentPhysicalCounts[key] : 0;
                ...
                if (currentCount < reservedCount)
                {
                    ...
                    if (extracted != null)
                    {
                        ...
                        {
                            Log.Message($"[数字存储] 补充预留物品: {extracted.Label} x{extracted.stackCount}, 当前={currentCount}, 需要={needed}");
                        }
                    }
                }
            }
        ...
        public Thing TryReplenishItem(ThingDef def, ThingDef stuff, int needed, QualityCategory quality)
        {
            ...
            else
            {
                if (DigitalStorageSettings.enableDebugLog)
                {
                    Log.Message($"[数字存储] TryReplenishItem: 虚拟存储不足，无法补货, 需要 {def.label} x{needed}");
                }
                return null;
            }
        }
        ...
        public void ConvertPhysicalItemsToVirtualWithReserved()
        {
            ...
            // 无条件输出：确认函数执行
            Log.Message($"[数字存储] 开始转换检查，预留数量: {reservedCount}");
            ...
            foreach (var kvp in totalCountsByKey)
            {
                ...
                Log.Message($"[数字存储] 统计对比: {def?.label ?? key}, 统计阶段总数: {statStageTotal}, 转换阶段总数: {totalCount}, 预留: {reservedCount}, 堆叠数: {currentThings.Count}, 差异: {totalCount - statStageTotal}");
                ...
                if (totalCount > reservedCount)
                {
                    ...
                    // 无条件输出：转换结果
                    ThingDef def2 = GetDefFromKey(key);
                    ThingDef stuff2 = GetStuffFromKey(key);
                    Log.Message($"[数字存储] 转换结果: {def2?.label ?? key}, 实际: {totalCount}, 预留: {reservedCount}, 需转换: {toConvertCount}, 已收集: {converted}, 差异: {toConvertCount - converted}");
                    
                    if (DigitalStorageSettings.enableDebugLog)
                    {
                        Log.Message($"[数字存储] [转换阶段] 转换收集完成: {def2?.label ?? key}, 实际: {totalCount}, 预留: {reservedCount}, 需转换: {toConvertCount}, 已收集: {converted}, 差异: {toConvertCount - converted}");
                    }
                }
                else
                {
                    // 无条件输出：未超出预留
                    ThingDef def3 = GetDefFromKey(key);
                    ThingDef stuff3 = GetStuffFromKey(key);
                    Log.Message($"[数字存储] 未超出预留: {def3?.label ?? key}, 实际: {totalCount}, 预留: {reservedCount}, 无需转换");
                    
                    if (DigitalStorageSettings.enableDebugLog)
                    {
                        Log.Message($"[数字存储] [转换阶段] 未超出预留: {def3?.label ?? key}, 实际: {totalCount}, 预留: {reservedCount}, 无需转换");
                    }
                }
            }
            ...
            {
                Log.Message($"[数字存储] 本次转换检查完成，无需转换任何物品");
                return;
            }
            
            Log.Message($"[数字存储] 准备转换 {toConvert.Count} 个堆叠");
            ...
            {
                Log.Message($"[数字存储] [详细] 本次转换总计: {totalConvertedCount} 个物品");
            }
