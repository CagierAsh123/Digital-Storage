...
namespace DigitalStorage.HarmonyPatches
{
    ...
    public static class Patch_Pawn_JobTracker_StartJob_Teleport
    {
        ...
        private static void HandleHaulToContainer(Job job, Pawn pawn)
        {
            ...
            if (DigitalStorageSettings.enableDebugLog)
            {
                Log.Message($"[数字存储] HandleHaulToContainer: 开始处理, pawn={pawn.LabelShort}, material={material.Label}, def={material.def.defName}, stackCount={material.stackCount}");
            }

            // ⚠️ 防止重复处理同一个 Job（使用同步）
            lock (jobExtractedItems)
            {
                if (jobExtractedItems.ContainsKey(job))
                {
                    if (DigitalStorageSettings.enableDebugLog)
                    {
                        Log.Message($"[数字存储] HandleHaulToContainer: Job已被处理过，跳过, {material.Label}, 已提取物品数量={jobExtractedItems[job].Count}");
                    }
                    return;
                }
            }

            // 如果物品已经生成到地图上（不是虚拟物品），说明已经处理过了
            bool isVirtualItem = false;
            if (material is ThingWithComps tempThingWithComps)
            {
                var comp = tempThingWithComps.GetComp<CompVirtualIngredient>();
                if (comp != null)
                {
                    isVirtualItem = comp.IsVirtual;
                }
            }

            if (material.Spawned && material.Map != null && !isVirtualItem)
            {
                if (DigitalStorageSettings.enableDebugLog)
                {
                    Log.Message($"[数字存储] HandleHaulToContainer: 物品已在地图上且不是虚拟物品，跳过, {material.Label}");
                }
                return;
            }

            // 额外检查：如果这个物品已经被处理过（有CompVirtualIngredient但不是虚拟的），跳过
            if (!isVirtualItem && material is ThingWithComps tempThingWithComps2)
            {
                var comp = tempThingWithComps2.GetComp<CompVirtualIngredient>();
                if (comp != null && !comp.IsVirtual)
                {
                    if (DigitalStorageSettings.enableDebugLog)
                    {
                        Log.Message($"[数字存储] HandleHaulToContainer: 物品已被处理过（有Comp但不是虚拟），跳过, {material.Label}");
                    }
                    return;
                }
            }
            ...
            // 只有在成功生成真实物品后才销毁虚拟物品
            if (virtualComp != null && virtualComp.IsVirtual && realThing != null)
            {
                // 销毁临时虚拟 Thing
                if (material.Spawned)
                {
    ...
    public static class Patch_Pawn_JobTracker_CleanupCurrentJob
    {
        public static void Postfix(JobCondition condition, Pawn ___pawn, Job ___curJob)
        {
            ...
            // 清理建造目标标记（如果这个Job是建造相关的）
            if (___curJob.def == JobDefOf.HaulToContainer && ___curJob.targetB.HasThing && ___curJob.targetB.Thing is IConstructible constructible)
            {
                // 调用清理方法
                Patch_WorkGiver_ConstructDeliverResources_V2.RemoveActiveConstructionTarget(constructible);

                if (DigitalStorageSettings.enableDebugLog)
                {
                    Log.Message($"[数字存储] Job结束，清理建造目标标记, target={constructible.ToString()}");
                }
            }
        }
