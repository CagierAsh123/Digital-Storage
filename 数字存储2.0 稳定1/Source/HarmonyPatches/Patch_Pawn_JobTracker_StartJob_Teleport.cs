...
namespace DigitalStorage.HarmonyPatches
{
    ...
    public static class Patch_Pawn_JobTracker_StartJob_Teleport
    {
        ...
        public static bool Prefix(Job newJob, Pawn ___pawn)
        {
            ...
            // 处理 HaulToContainer（蓝图建造）- 新架构：先补货，再传送
            else if (newJob.def == JobDefOf.HaulToContainer && newJob.targetA.HasThing && newJob.targetB.HasThing)
            {
                bool shouldBlock = HandleHaulToContainer(newJob, ___pawn);
                return !shouldBlock; // 如果传送成功，阻止原版逻辑
            }
        ...
        private static void HandleDoBill(Job job, Pawn pawn)
        {
            ...
            for (int i = 0; i < job.targetQueueB.Count; i++)
            {
                ...
                // 检查是否来自预留物品
                if (virtualComp.ReservedThingPositionOnly && virtualComp.ReservedThing != null)
                {
                    // 预留物品仅作为位置提供者，完全从虚拟存储提取
                    if (realThing == null)
                    {
                        ...
                        {
                            Log.Message($"[数字存储] HandleHaulToContainer: 预留物品仅作为位置提供者，完全从虚拟存储提取, {material.def?.label ?? "null"} x{neededCount}, sourceCore={(sourceCore != null ? "存在" : "null")}");
                        }
                ...
                else if (virtualComp.IsFromReserved && virtualComp.ReservedThing != null)
                {
        ...
        /// </summary>
        private static bool HandleHaulToContainer(Job job, Pawn pawn)
        {
            if (!job.targetA.HasThing || job.targetA.Thing == null || !job.targetB.HasThing)
            {
                return false; // 不阻止原版逻辑
            }

            Thing material = job.targetA.Thing;
            Thing destination = job.targetB.Thing; // 蓝图
            ...
            // 简化逻辑：如果物品已经在目标位置（蓝图位置），说明已经在WorkGiver阶段传送过了
            // 直接返回true，让pawn去拿，不需要再传送
            if (material.Position == destination.Position && material.Map == destination.Map)
            {
                if (DigitalStorageSettings.enableDebugLog)
                {
                    Log.Message($"[数字存储] HandleHaulToContainer: 物品已在目标位置，无需传送, pawn={pawn.LabelShort}, material={material.Label}, 位置={material.Position}");
                }
                return true; // 阻止原版逻辑，pawn直接去拿
            }

            // 如果物品不在目标位置，可能是预留物品或其他情况
            ...
            // 检查存储设置是否允许该物品
            if (!materialCore.GetStoreSettings().AllowedToAccept(material))
            {
                if (DigitalStorageSettings.enableDebugLog)
                {
                    Log.Message($"[数字存储] HandleHaulToContainer: 存储设置不允许该物品, {material.Label}");
                }
                return false; // 不允许，让原版逻辑处理
            }

            // 这是预留物品，需要传送（兜底逻辑，正常情况下不应该到这里）
            if (DigitalStorageSettings.enableDebugLog)
            {
                Log.Message($"[数字存储] HandleHaulToContainer: 检测到预留物品，执行传送, pawn={pawn.LabelShort}, material={material.Label}, 数量={material.stackCount}, 目标={destination.Label}");
            }
            ...
            lock (jobExtractedItems)
            {
                if (jobExtractedItems.ContainsKey(job))
                {
                    return true; // 已处理过，阻止原版逻辑
                }
                jobExtractedItems[job] = new List<Thing>();
            }

            // 物品传送仪式逻辑：直接传送物品到目标位置
            ...
            {
                // 有芯片：传送到蓝图位置
                teleportPos = destination.Position;
            }
            else
            {
                // 无芯片：查找最近的输出接口位置，并比较接口和核心的距离
                Building_OutputInterface nearestInterface = FindNearestOutputInterface(pawn.Map, materialCore);
                if (nearestInterface != null && nearestInterface.Spawned)
                {
                    // 计算接口到材料位置的距离
                    float interfaceDistance = (nearestInterface.Position - material.Position).LengthHorizontalSquared;
                    // 计算核心到材料位置的距离
                    float coreDistance = (materialCore.Position - material.Position).LengthHorizontalSquared;
                    
                    if (coreDistance <= interfaceDistance)
                    {
                        // 核心更近，不传送，让pawn走过去拿
                        if (DigitalStorageSettings.enableDebugLog)
                        {
                            Log.Message($"[数字存储] HandleHaulToContainer: 核心更近，不传送, 核心距离={coreDistance}, 接口距离={interfaceDistance}");
                        }
                        return false; // 不阻止原版逻辑
                    }
                    
                    // 接口更近，传送到接口位置
                    teleportPos = nearestInterface.Position;
                }
                else
                {
                    teleportPos = destination.Position; // 兜底：使用蓝图位置
                }
            }

            // 处理堆栈
            Thing thingToTeleport;
            if (material.stackCount <= neededCount)
            {
                // 完整堆栈传送
                material.DeSpawn(0);
            ...
            {
                // 部分堆栈传送
                thingToTeleport = material.SplitOff(neededCount);
            }

            // 传送到目标位置
            GenSpawn.Spawn(thingToTeleport, teleportPos, pawn.Map, 0);
            ...
            if (DigitalStorageSettings.enableDebugLog)
            {
                Log.Message($"[数字存储] HandleHaulToContainer: 传送完成, {thingToTeleport.Label} x{thingToTeleport.stackCount} 从 {material.Position} 到 {teleportPos}");
            }

            // 返回true表示传送成功，应该阻止原版逻辑（类似物品传送仪）
            return true;
        }
        ...
        private static Building_OutputInterface FindNearestOutputInterface(Map map, Building_StorageCore core)
        {
            ...
            {
                if (iface == null || !iface.Spawned || !iface.IsActive || iface.BoundCore != core)
                {
                    continue;
                }
