...
namespace DigitalStorage.Components
{
    ...
    public class Building_InputInterface : Building_Storage
    {
        ...
        {
            Log.Message($"[InputInterface] SpawnSetup 开始, map={map}, respawningAfterLoad={respawningAfterLoad}");
            try
            {
                ...
                Log.Message($"[InputInterface] SpawnSetup 完成");
            }
            catch (System.Exception ex)
            {
                Log.Error($"[InputInterface] SpawnSetup 异常: {ex}");
            }
        }

        public override void DeSpawn(DestroyMode mode = DestroyMode.Vanish)
        {
            Log.Message($"[InputInterface] DeSpawn 开始");
            try
            {
                SetBoundCore(null);
                base.DeSpawn(mode);
                Log.Message($"[InputInterface] DeSpawn 完成");
            }
            catch (System.Exception ex)
            {
                Log.Error($"[InputInterface] DeSpawn 异常: {ex}");
            }
        }

        public override void ExposeData()
        {
            Log.Message($"[InputInterface] ExposeData 开始, mode={Scribe.mode}");
            try
            {
                base.ExposeData();
                Scribe_References.Look(ref boundCore, "boundCore");
                Log.Message($"[InputInterface] ExposeData 完成, boundCore={boundCore}");
            }
            catch (System.Exception ex)
            {
                Log.Error($"[InputInterface] ExposeData 异常: {ex}");
            }
        }
