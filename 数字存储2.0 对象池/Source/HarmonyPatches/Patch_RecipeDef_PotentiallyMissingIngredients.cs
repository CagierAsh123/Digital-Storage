...
namespace DigitalStorage.HarmonyPatches
{
    ...
    public static class Patch_RecipeDef_PotentiallyMissingIngredients
    {
        public static void Postfix(ref IEnumerable<ThingDef> __result, RecipeDef __instance, Pawn billDoer, Map map)
        {
            ...
            // 注意：我们需要检查虚拟存储中是否有足够的数量可以补货
            // 因为TryFindBestBillIngredients会先补货，所以这里应该检查补货后的情况
            foreach (IngredientCount ingredient in __instance.ingredients)
            {
                ...
                foreach (ThingDef allowedDef in ingredient.filter.AllowedThingDefs)
                {
                    ...
                    // 检查虚拟存储中是否有足够的数量（使用GetItemCount，包括预留物品和虚拟存储）
                    ...
                    foreach (Building_StorageCore core in gameComp.GetAllCores())
                    {
                        ...
                        // 使用GetItemCount检查总数量（包括预留物品和虚拟存储）
                ...
                if (!foundInStorage)
                {
                    ...
                    {
                        // 如果是固定材料，直接添加第一个允许的材料类型（与原版逻辑一致）
                        ThingDef fixedDef = ingredient.filter.AllowedThingDefs.FirstOrDefault();
                        if (fixedDef != null && !stillMissing.Contains(fixedDef))
                        {
                            stillMissing.Add(fixedDef);
                        }
                    }
                    else
                    {
                        // 如果不是固定材料，从missingIngredients中查找对应的缺失材料
